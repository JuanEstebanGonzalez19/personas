...
methods: {
    ...
    async updateComuna(){
        const res = await axios.put(`https://127.0.0.1:8000/api/comunas/${this.comuna.comu_codi}`,this.comuna)
        if (res.status == 200){
            this.$router.push({name:'Comunas'})
            Swal.fire({
                position: 'top-end',
                icon: 'success',
                title: 'Comuna has been update',
...
mounted(){
    ...
    axios.get(`https://127.0.0.1:8000/api/comunas/${this.comuna.comu_codi}`)
    .then(response => {
